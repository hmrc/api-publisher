...
```json
{
  ...
  "message": "Unable to parse request body : {\"obj.serviceName\":[{\"msg\":[\"error.path.missing\"],\"args\":[]}]}"
}
```
...
Documentation for this schema is generated by:

```bash
sbt "generateDoc app/resources/api-definition-schema.json docs/api-definition.md"
```

which is included for convenience in a script `generateDoc.sh` and at the end of `run_all_tests.sh`.

**This needs to be run whenever the schema is updated and the generated file should be committed. The MDTP Handbook links to the generated documentation.**
